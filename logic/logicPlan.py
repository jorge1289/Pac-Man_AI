...
def sentence1() -> Expr:
    ...
    return conjoin(a_or_b, not_a, not_a_or_not_b_or_c)
    "*** END YOUR CODE HERE ***"
...
def sentence2() -> Expr:
    ...
    c_if = C % (B | D)
    a_implies = A >> (~B & ~D)
    not_b_and_not_c = ~(B & ~C) >> A
    not_d = ~D >> C

    return conjoin(c_if, a_implies, not_b_and_not_c, not_d)
...
def sentence3() -> Expr:
    ...
    PacmanKilled = PropSymbolExpr("PacmanKilled_0")
    PacmanBorn = PropSymbolExpr("PacmanBorn_0")

    a = PacmanAlive_1 % (PacmanAlive_0 & ~PacmanKilled | ~PacmanAlive_0 & PacmanBorn)
    b = ~(PacmanAlive_0 & PacmanBorn)
    c = PacmanBorn
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    lst = list((itertools.combinations(literals, 2)))
    
    new_list = []
    for item in lst:
        new_list.append(conjoin(~item[0] | ~item[1]))
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    for item in lst:
        new_list.append(conjoin((~item[0] | ~item[1])))
    return conjoin(new_list) & disjoin(literals)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #previous = PropSymbolExpr(pacman_str, x, y, time=last)
    action = disjoin(possible_causes)
    other = action
    return PropSymbolExpr(pacman_str, x, y, time=now) % other
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for (x, y) in non_outer_wall_coords: 
        possible.append(PropSymbolExpr(pacman_str, x, y, time=t))

    for direction in DIRECTIONS:
        current.append(PropSymbolExpr(direction, time = now))
    ## Make sure Pacman is at exactly one of the squares at timestep t.
    ## Make sure pacman can one take one actions at one timestep
    if len(possible) != 0:
        pacphysics_sentences.append(exactlyOne(possible))
    if len(current) != 0:
        pacphysics_sentences.append(exactlyOne(current)) 


    if sensorModel != None:
        pacphysics_sentences.append(sensorModel(now, non_outer_wall_coords))
    if successorAxioms != None and now !=0:
        pacphysics_sentences.append(successorAxioms(now, walls_grid, non_outer_wall_coords))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #model1 = PropSymbolExpr(pacman_str, x1, y1, time=1) & PropSymbolExpr(pacman_str, x0, y0, time=1) & PropSymbolExpr(action0, time=1) & PropSymbolExpr(action1, time=1)
    
    
    #model2 = ~PropSymbolExpr(pacman_str, x1, y1, time=1) & PropSymbolExpr(pacman_str, x0, y0, time=1) & PropSymbolExpr(action0, time=1) & PropSymbolExpr(action1, time=1)
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for (x, y) in non_wall_coords: 
            possible.append(PropSymbolExpr(pacman_str, x, y, time=t))
            # make sure pacman is not at the place when a wall here
            KB.append(PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, time = t))
        KB.append(exactlyOne(possible))

        for direction in DIRECTIONS:
            current.append(PropSymbolExpr(direction, time = t))
        KB.append(exactlyOne(current))

        if t > 0:
            for (x,y) in non_wall_coords:
                KB.append(pacmanSuccessorAxiomSingle(x, y, t, walls_grid))

        var = findModel(PropSymbolExpr(pacman_str, xg, yg, time=t) & conjoin(KB))

        if var:
            return extractActionSequence(var, actions)
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    # Where is the location of all food
    for (x, y) in food:
    ...
    for t in range(50):
        ...
        # Food successor axiom: relation between Food[x, y]_t+1 and Food[x,y]_t and Pacman[x,y]_t
        for (x, y) in food:
            # if Pacman is at (x, y) at time t, and there is a food at (x, y) at time t, then there will be no food at (x, y) at time t+1
            s1 = PropSymbolExpr(food_str, x, y, time=t) & PropSymbolExpr(pacman_str, x, y, time=t)
            # There is no food at (x, y) at time t
            s2 = ~PropSymbolExpr(food_str, x, y, time=t)
            KB.append(~PropSymbolExpr(food_str, x, y, time=t+1) % disjoin(s1, s2))

        # Pacman Successor axiom: all possible position in time t+1
        for (x, y) in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x, y, t+1, walls))

        for (x, y) in food:
            food_rest.append(PropSymbolExpr(food_str, x, y, time=t))
        # This happens when all Food[x,y]_t are false.
        goal_assertion = disjoin(food_rest)

        var = findModel(conjoin(KB) & ~goal_assertion)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for (x ,y) in non_outer_wall_coords:
            model1 = entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time = t)) # model of there is a pacman at time t
            model2 = entails(conjoin(KB), ~PropSymbolExpr(pacman_str, x, y, time = t)) # model of there not
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for (x ,y) in non_outer_wall_coords:
            model1 = entails(conjoin(KB), PropSymbolExpr(wall_str, x, y)) # model of there is a wall at time t
            model2 = entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y)) # model of there not
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for (x ,y) in non_outer_wall_coords:
            wallmodel_1 = entails(conjoin(KB), PropSymbolExpr(wall_str, x, y)) # model of there is a wall at time t
            wallmodel_2 = entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y)) # model of there not
