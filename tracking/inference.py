...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            l.append((x, y))
    variableDomainsDict[PAC] = l
    variableDomainsDict[GHOST0] = l
    variableDomainsDict[GHOST1] = l
    ...
    variableDomainsDict[OBS0] = temp
    variableDomainsDict[OBS1] = temp
    # raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in self.allPositions:
            ...
            self.beliefs[p] = self.getObservationProb(observation, gameState.getPacmanPosition(),p, self.getJailPosition()) * belief
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        h  = DiscreteDistribution()
        for position in self.allPositions:
            var = self.getPositionDistribution(gameState, position)
